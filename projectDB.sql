...
CREATE TABLE IF NOT EXISTS `mydaily`.`user` (
  `email` VARCHAR(45) NOT NULL COMMENT '사용자 아이디',
...
CREATE TABLE IF NOT EXISTS `mydaily`.`board` (
  ...
  `tag` TEXT NOT NULL,
  `board_write_time` DATETIME NOT NULL DEFAULT now() COMMENT '게시물 작성 시간',
  ...
  INDEX `fk_board_user1_idx` (`writer_email` ASC),
...
CREATE TABLE IF NOT EXISTS `mydaily`.`product` (
  `product_number` INT NOT NULL AUTO_INCREMENT COMMENT '상품 등록 번호',
  `product_name` TEXT NULL DEFAULT NULL COMMENT '상품 이름',
  `product_price` INT NULL DEFAULT NULL COMMENT '상품 가격',
  `product_url` TEXT NULL DEFAULT NULL COMMENT '상품 구매 링크',
  `product_img_url` TEXT NULL DEFAULT NULL COMMENT '상품 이미지',
...
CREATE TABLE IF NOT EXISTS `mydaily`.`comment` (
  ...
  INDEX `fk_comment_board1_idx` (`board_number` ASC),
  INDEX `fk_comment_user1_idx` (`writer_email` ASC),
  UNIQUE INDEX `comment_number_UNIQUE` (`comment_number` ASC),
...
CREATE TABLE IF NOT EXISTS `mydaily`.`liky` (
  ...
  INDEX `fk_liky_board1_idx` (`board_number` ASC),
  INDEX `fk_liky_user1_idx` (`user_email` ASC),
...
CREATE TABLE IF NOT EXISTS `mydaily`.`board_has_product` (
  ...
  INDEX `fk_board_has_product_product1_idx` (`product_number` ASC),
  INDEX `fk_board_has_product_board1_idx` (`board_number` ASC),
